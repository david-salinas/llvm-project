...
static void llvm::objcopy::parseDumpURI(StringRef URI) {
...
objcopy::parseObjcopyOptions(ArrayRef<const char *> ArgsArr,
  ...
  if (InputArgs.hasArg(OBJCOPY_dump_URI))
  ...
  if (Arg *A = InputArgs.getLastArg(OBJCOPY_dump_URI)) {
      for (StringRef URIStr : llvm::split(A->getValue(), ",")) {
        llvm::objcopy::parseDumpURI(URIStr);
      }
  }
